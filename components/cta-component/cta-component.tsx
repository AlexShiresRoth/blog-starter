...
import { CtaComponentType } from '@/types/cta-component/cta-component.type';

type Props = {
  id: string;
};

interface CtaComponentResponseData {
  data: {
    componentCta: CtaComponentType;
  };
}
async function getComponent(id: string) {
  try {
    const res = await fetchGraphQL<CtaComponentResponseData>(ctaQuery(id));
